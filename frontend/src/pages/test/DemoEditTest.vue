<template>
  <div>
    <StandardLayout v-if="completed">
      <v-card :class="$style.card">
        <v-card-title class="title">
          {{ test.name }}
        </v-card-title>
        <v-card-text class="subheading text-xs-left px-5">
        ...
        <v-layout mt-4>
          <v-flex xs12>
            ...
            <v-btn flat color="info" @click="editToggle">
              Re-edit this test
            </v-btn>
    ...
    <TestLayout v-else :sections="sections" :sectionIndex="sectionIndex">
...
export default {
  data() {
    return {
      ...
      test: localStorage.getItem("demoTest")
        ? JSON.parse(localStorage.getItem("demoTest"))
        : demoTest
    };
  },
  components: {
  ...
  methods: {
    ...
    editToggle() {
      this.test.completed = !this.test.completed;
    },
    addSection(name) {
      this.sections.push({
        subject: name,
  ...
  watch: {
    ...
    test: {
      ...
      handler(newTest, oldTest) {
        localStorage.demoTest = JSON.stringify(newTest);
      }
    }
  },
  computed: {
    completed() {
      return this.test.completed;
    },
    subjects() {
  ...
  created() {
    if (!localStorage.getItem("demoTest"))
      localStorage.setItem("demoTest", JSON.stringify(demoTest));
  },
  mounted() {}
};
</script>
