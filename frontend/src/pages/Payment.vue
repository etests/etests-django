    ...
    <v-card flat :class="$style.dialog">
      <template v-if="!help">
        <v-card-title :class="$style.title">
          Buy {{ testSeries.name }}
          <v-spacer/>
          <v-btn round outline color="primary" v-on:click="toggleHelp()">Help</v-btn>
        ...
        <v-card-text :class="$style.subheading">
          Make a payment of &#8377;{{ testSeries.price }} using one of these methods and submit transaction id and receipt after successful payment.The demo of receipt are shown in help.
        </v-card-text>
        <v-layout row wrap>
          <v-flex xs12 md6>
              <div :class="$style.subheading">Scan and Pay Using Google Pay</div>
            <v-img :src="require(`@/assets/images/payment/PankajQRcode.jpg`)" class="ma-auto" width="300px" />
          </v-flex>
          <v-flex xs12 md6>
          ...
          <v-flex xs12>      
              <form enctype="multipart/form-data">
                ...
                <input type="file" v-show="false" id="receipt" ref="receipt" v-on:change="handleFileUpload()" />
                <div class="text-xs-left">
                <v-btn round outline small color="primary" @click="$refs.receipt.click()">
                  <label for="receipt">choose receipt</label>
                </v-btn> <span> {{receipt.name}}</span>
                </div>
              </form>
          </v-flex>
        </v-layout>
      ...
      <template v-else>
        <v-card-title :class="$style.title">
          The receipts for various payment methods will look like these. <br/>
          <v-spacer />
          <v-btn outline round color="primary" v-on:click="toggleHelp()">
            back
          </v-btn>
          <div>
          <b>Note: The transaction id is encircled <span class="red--text">red</span>.</b>
          </div>
        </v-card-title>
...
export default {
  ...
  data() {
    return {
      ...
      receipt: "",
       cards: [
        { title: 'Google Pay', receipt_src:require(`@/assets/images/payment/gpay.jpg`), flex: 4 },
        { title: 'PhonePe',receipt_src:require(`@/assets/images/payment/phonepe.jpg`), flex: 4 },
        { title: 'Netbanking', receipt_src:require(`@/assets/images/payment/netbanking.jpg`), flex: 4 }
      ]
    };
  },
  methods: {
