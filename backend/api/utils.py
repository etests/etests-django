    ...
    def evaluate(self):
        for i in range(len(self.questions)):
            if self.questions[i]["type"] == 3:
                self.maxMarks[-1] += self.questions[i]["partialMarks"] * len(
                    self.test.answers[i]["answer"]
                )
                self.maxMarks[self.questions[i]["section"]] += self.questions[i][
                    "partialMarks"
            ...
            if self.questions[i]["type"] == 0:
                ...
                if self.questions[i]["partialMarks"] > 0 and all(
                ...
                for j in range(len(responses)):
                    if len(responses[j]) == 0:
                        continue
                    elif len(responses[j]) != 0 and sorted(responses[j]) == sorted(
                        answers[j]
                    ):
                        curMarks[j]["marks"] = self.questions[i]["partialMarks"]
                        curMarks[j]["status"] = 2
                    else:
                        curMarks[j]["marks"] = (self.questions[i]["incorrectMarks"]) * (
                            -1
                        )
                        curMarks[j]["status"] = 1
...
def generateRanks(sessions):
    ...
    sectionwise_marks_list = [
        [[session.marks["sectionWise"][j], i] for (i, session) in enumerate(sessions)]
        for j in range(len(sessions[0].marks["sectionWise"]))
    ]
    marks_list.sort()
    marks_list.reverse()
    ...
    return {
        ...
        "stats": {
            "average": {
                ...
                "sectionWise": [
                    sectionTotal / len(sessions) for sectionTotal in sectionWiseTotal
                ],
            },
            "highest": {
                "overall": marks_list[0][0],
                "sectionWise": [
                    section_marks[0][0] for section_marks in sectionwise_marks_list
                ],
            },
        },
        "marks_list": {"overall": marks_list, "sectionWise": sectionwise_marks_list},
    }
